...
import { useAuth } from "../contexts/AuthContext";
import {
  Menu, Moon, Sun, Globe, X, Mail, Phone, ChevronDown,
...
const Header = () => {
  ...
  return (
    <header className="w-full">
      ...
      {/* Main Nav */}
      <div className={`bg-white dark:bg-gray-900 dark:text-white text-black py-4 px-4 fixed top-9 left-0 w-full z-40 transition-shadow duration-300 ${scrolled ? "shadow-lg" : ""}`}>
        <div className="container mx-auto flex justify-between items-center">
          <Logo />
          <nav className="hidden md:flex items-center gap-6">
            ...
            <div className="relative">
              ...
              <div className={`absolute top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg transition-all ${servicesOpen ? "opacity-100 max-h-96" : "opacity-0 max-h-0 overflow-hidden"}`}>
                {["Car Rides", "Rentals", "Auto Rides", "Bike Rides", "Intercity"].map(service => (
                  <Link key={service} to={`/Services/${service.replace(/\s+/g, "_")}`} className="block px-4 py-2 hover:bg-green-200 dark:hover:bg-green-700">
                    {service}
                  </Link>
                ))}
              </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {isAuthenticated ? (
              ...
              <>
                <Link to="/login" className="bg-[#0c0d0f] text-sm text-white border border-gray-600 px-4 py-2 rounded-md hover:bg-green-900">Login</Link>
                <Link to="/signup" className="bg-green-500 text-sm text-white px-4 py-2 rounded-md hover:bg-green-700">Sign Up</Link>
              </>
            )}
          </div>
      ...
      <div className={`fixed inset-0 bg-white dark:bg-gray-900 z-30 pt-28 px-4 transition-transform duration-300 ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"} md:hidden`}>
        <nav className="flex flex-col space-y-4">
          ...
          <Link to="/Services/carrides" onClick={() => setMobileMenuOpen(false)}>Car Rides</Link>
          <Link to="/Services/rentals" onClick={() => setMobileMenuOpen(false)}>Rentals</Link>
